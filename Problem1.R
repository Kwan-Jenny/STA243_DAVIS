...
M = 15 # #.of times to anneal

k_max = 2000 # steps in the annealing process
...
nhbd = function(s, k = 2){
  # Generate a neighborhood around path 
...
results = data.frame(Step=integer(),Length=integer(),M=integer(),stringsAsFactors=F)
...
for(m in 1:M){
  ...
  for(k in 1:k_max){
    ...
    h0 = path_leng(s0)    # Calculate energy of object
    h1 = path_leng(s1)
    ...
    # record results
    results[dim(results)[1]+1,] = c(k, path_leng(s0), m)
    
  }
  best_path[[length(best_path)+1]] = s0
}

results = results[results$Step!=k_max,]

bp = best_path[[results$M[tail(which(results$Length==min(results$Length)),1)]]]

## Plot result

  ggplot(results, aes(x=Step, y=Length, color=factor(M))) +
  geom_line() +
  ggtitle("Traveling Salesman problem") +
